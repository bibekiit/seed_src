...
def tps_iter_match_1(m1, m2, X, Y, orients, nbins_theta, nbins_r, r_inner, r_outer, n_iter, r, beta_init, i1, i2, anchors):
    ...
    while s:
        ...
        #print np.tile(cx[n_good+1:n_good+3],(1,1))
        #print A[1,0]
        #print A[0,0]
        ...
        #print (np.dot(math.atan2(V[0, 1], V[0, 0]), 180) / np.pi)
        #print (np.dot(math.atan2(U[0, 1], U[0, 0]), 180) / np.pi)
    ...
    if abs(angle) < MAX_ANGLE and E < 15 and aff_cost < 0.3:
